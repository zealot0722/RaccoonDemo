...
- 系統會偵測 `multi_intent`，並把可處理的 FAQ、查貨態、商品推薦與退貨資料追問拆成分段回覆。

設計：

- 多意圖不再一律轉人工。
- 若包含查貨態，先查 `order_statuses`；缺編號時先追問訂單編號或物流單號。
- 若同時包含退貨，先要求送貨貨號、姓名、電話號碼；資料齊全後才轉人工判斷退貨。
- 若同時包含 FAQ 或商品推薦，先回答可命中的 FAQ，並只推薦符合 DB 價格條件的商品。
- 若包含明確真人客服或客訴，仍建立 `needs_review` 並把另一個需求寫入客服摘要。
